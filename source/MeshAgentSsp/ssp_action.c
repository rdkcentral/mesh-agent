...
ANSC_STATUS ssp_engage(PCCSP_COMPONENT_CFG pMeshCfg)
{
    ...
    if ( g_Subsystem[0] != 0 )
    {   /*CID 72310 calling risky function*/
...
ANSC_STATUS ssp_cancel(PCCSP_COMPONENT_CFG pMeshCfg)
{
    ...
    if ( g_Subsystem[0] != 0 )
    {   /*CID 56843 calling risky function*/
        snprintf(CrName, sizeof(CrName), "%s%s", g_Subsystem, CCSP_DBUS_INTERFACE_CR);
        snprintf(CpName, sizeof(CpName), "%s%s", g_Subsystem, pMeshCfg->ComponentName);
    }
    else
    {
        snprintf(CrName, sizeof(CrName), "%s", CCSP_DBUS_INTERFACE_CR);
        snprintf(CpName, sizeof(CpName), "%s", pMeshCfg->ComponentName);
    }
